...
EzMongo.prototype.insert = function(collectionName, docs, callback) {
    ...
    async.auto({
        ...
        var inserted_ids = results && results.insert && results.insert.ops && _.map(results.insert.ops, function(insertedDoc) {
...
function _modifyBase(self, collectionName, _idOrSearch, changes, multiple, callback) {
    ...
    async.auto({
        ...
        modify: ['collection', function(next, results) {
            ...
            if (multiple) {
                collection.updateMany(searchObj, changes, optionsObj, next);
            } else {
                collection.updateOne(searchObj, changes, optionsObj, next);
            }
        }]
    }, function(err, results) {
        var numberModified = results && results.modify && results.modify.result && results.modify.result.nModified || 0;
